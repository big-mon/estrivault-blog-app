...
これは下書き記事です。`);
  ...
  describe('getAllPosts', () => {
    it('指定されたパターンに一致するMarkdownファイルからメタデータ一覧を取得する', async () => {
      const posts = await getAllPosts(path.join(tempDir, '*.md'));
    ...
    it('フィルタオプションを使用して特定の記事だけを取得できる', async () => {
      // 下書きでない記事だけを取得
      const publishedPosts = await getAllPosts(path.join(tempDir, '*.md'), {
      ...
      // 特定のカテゴリの記事だけを取得
      const testCategoryPosts = await getAllPosts(path.join(tempDir, '*.md'), {
    ...
    it('ソートオプションを使用して記事を並べ替えできる', async () => {
      // タイトル順にソート
      const titleSortedPosts = await getAllPosts(path.join(tempDir, '*.md'), {
        sort: 'title'
      });
      
      expect(titleSortedPosts[0].title).toBe('下書き記事');
      expect(titleSortedPosts[1].title).toBe('記事1');
      expect(titleSortedPosts[2].title).toBe('記事2');
      expect(titleSortedPosts[3].title).toBe('記事3');
    });
    
    it('ページネーションオプションを使用して記事を分割できる', async () => {
      // 1ページ目（2件）
      const page1 = await getAllPosts(path.join(tempDir, '*.md'), {
        page: 1,
        perPage: 2
      });
      ...
      // 2ページ目（2件）
      const page2 = await getAllPosts(path.join(tempDir, '*.md'), {
        page: 2,
        perPage: 2
      });
  ...
  describe('getPostBySlug', () => {
    it('指定されたslugに一致するMarkdownファイルを処理してHTMLとメタデータを返す', async () => {
      const post = await getPostBySlug('post1', tempDir);
