...
- Canonical URL policy:
  ...
  - Archive pages keep the trailing slash, for example `/category/[category]/` and `/notes/`
...
### Notes and Contextual Modals

- Notes live under `content/notes/` and are short, independent mini-articles, not social timeline posts.
- Note frontmatter is intentionally minimal. Use only `title`, `publishedAt`, and `tags`; do not add category, excerpt, media, thumbnail, description, or slug fields unless the user explicitly changes the content model.
- Note body media belongs in Markdown content. There is no uniform note thumbnail field.
- Note cards must remain normal links (`<a href="/notes/[slug]">`) so the site works without JavaScript.
- JavaScript may progressively enhance note links into route-backed modal behavior. Opening from an archive context should push `/notes/[slug]`; closing should return to the clean archive URL such as `/notes/` or `/`, without preserving query strings unless there is a clear product need.
- Direct access to `/notes/[slug]` must render as a normal standalone page. Do not make note content readable only through a modal.
- Keep modal accessibility in place when changing this flow: dialog semantics, Esc close, outside click, focus movement/trap, background scroll lock, and focus restoration.
- If the in-app browser is using an `astro preview` server, rebuild after source changes before relying on visual verification.
