...
- Redirects to canonical URLs are generated by `apps/astro-blog/src/pages/_redirects.ts` as `dist/_redirects`.
  This intentionally relies on Cloudflare Workers Static Assets `_redirects` support for HTTP 301 redirects because Astro static redirects do not cover hosting-level trailing slash canonicalization as real 301 responses. Post trailing-slash redirects are generated as static per-post rules because all post slugs are known at build time.
