<script lang="ts">
  ...
  // OGP用のデータを動的生成
  const pageTitle = $derived(`${data.category.toUpperCase()}の記事一覧${data.pagination.page > 1 ? ` - ページ${data.pagination.page}` : ''} | ${SITE_TITLE}`);
  const pageDescription = $derived(`${data.category.toUpperCase()}カテゴリーの記事一覧${data.pagination.page > 1 ? `（${data.pagination.page}ページ目）` : ''}です。全${data.pagination.total}件の記事を掲載しています。`);
...
<svelte:head>
  ...
  {#if data.pagination.page > 1}
    <link rel="prev" href={`${SITE_URL}/category/${data.category}/${data.pagination.page > 2 ? data.pagination.page - 1 : '1'}`} />
  {/if}
...
<main class="container mx-auto px-4 py-8">
  <div class="mb-8 text-center">
    ...
    <p class="text-gray-600">
      全{data.pagination.total}件の記事（{data.pagination.page} / {data.pagination.totalPages}ページ）
    </p>
  </div>
