<script lang="ts">
  ...
  // OGP用のデータを動的生成
  const pageTitle = $derived(`${pageData.tag}の記事一覧${pageData.pagination.page > 1 ? ` - ページ${pageData.pagination.page}` : ''} | ${SITE_TITLE}`);
  const pageDescription = $derived(`${pageData.tag}タグが付けられた記事一覧${pageData.pagination.page > 1 ? `（${pageData.pagination.page}ページ目）` : ''}です。全${pageData.pagination.total}件の記事を掲載しています。`);
  const pageUrl = $derived(`${SITE_URL}/tag/${pageData.tag}/${pageData.pagination.page}`);
</script>
...
<svelte:head>
  ...
  {#if pageData.pagination.page > 1}
    <link rel="prev" href={`${SITE_URL}/tag/${pageData.tag}/${pageData.pagination.page > 2 ? pageData.pagination.page - 1 : '1'}`} />
  {/if}
  {#if pageData.pagination.page < pageData.pagination.totalPages}
    <link rel="next" href={`${SITE_URL}/tag/${pageData.tag}/${pageData.pagination.page + 1}`} />
  {/if}
</svelte:head>
