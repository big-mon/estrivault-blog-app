<script lang="ts">
  ...
  // OGP用のデータを動的生成
  const pageTitle = $derived(`${data.tag}の記事一覧${data.pagination.page > 1 ? ` - ページ${data.pagination.page}` : ''} | ${SITE_TITLE}`);
  const pageDescription = $derived(`${data.tag}タグが付けられた記事一覧${data.pagination.page > 1 ? `（${data.pagination.page}ページ目）` : ''}です。全${data.pagination.total}件の記事を掲載しています。`);
...
<svelte:head>
  ...
  {#if data.pagination.page > 1}
    <link rel="prev" href={`${SITE_URL}/tag/${data.tag}/${data.pagination.page > 2 ? data.pagination.page - 1 : '1'}`} />
  {/if}
