...
${posts.map((post: { title: string; slug: string; publishedAt: string; category: string; description?: string }) => `- [${post.title}](${SITE_URL.replace(/\/$/, '')}/post/${post.slug}) (${new Date(post.publishedAt).toISOString().split('T')[0]}) [${post.category}] - ${post.description || '記事の詳細な解説'}`).join('\n')}
